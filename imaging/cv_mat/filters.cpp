...
static std::pair< functor_type, bool > make_filter_functor( const std::vector< std::string >& e, const get_timestamp_functor& get_timestamp, bool allow_parallelisation )
{
    ...
    if( e[0] == "overlay" )
    {
        COMMA_ASSERT( e.size() == 2, "overlay: expected file name (and optional x ,y) with the overlay, e.g. overlay=a.svg" );
        std::vector< std::string > s = comma::split( e[1], ',' );
        COMMA_ASSERT( s.size() == 1 || s.size() == 3, "expected one or three parameters overlay=<file>[,<x>,<y>]; got: '" << e[1] << "'" );
        int x = 0, y = 0;
...
struct composer
{
    ...
    {
        return std::make_pair( [ op ]( const input_type & i ) -> input_type { const input_type & o = op.first( i ); composer::assert_integer( o ); return std::make_pair( i.first, ~o.second ); }, op.second );
    }

    static void assert_integer( const input_type & i )
    {
        COMMA_ASSERT( i.second.depth() != CV_32F && i.second.depth() != CV_64F, "bitwise operations shall be done on integer inputs" );
    }
};
...
std::vector< typename impl::filters< H >::filter_type > impl::filters< H >::make( const std::string& how, const get_timestamp_functor& get_timestamp, bool allow_parallelisation )
{
    ...
    for( std::size_t i = 0; i < v.size(); name += ( i > 0 ? ";" : "" ) + v[i], ++i )
    {
        ...
        else if( e[0] == "multiply" || e[0] == "divide" || e[0] == "add" || e[0] == "subtract" || e[0] == "absdiff" || e[0] == "minimum" || e[0] == "maximum" )
        {
...
//         }
        ...
        {
            COMMA_ASSERT( e.size() >= 2, "pack: please specify pack parameters" );
            const std::vector< std::string >& args = comma::split( e[1], ',' );
            if( args[0] == "12" )
            {
...
static std::string usage_impl_()
{
    ...
    oss << "    accumulate=<n>[,<how>][,<options>]: accumulate the last n images and concatenate them vertically\n";
    oss << "                                        (useful for slit-scan and spectral cameras like pika2)\n";
    ...
    oss << "            default: sliding\n";
    oss << "            example: run the command line below; press white space key to see image accumulating;\n";
    oss << "                     try it with fixed or fixed-inverse\n";
    oss << "                $ ( yes 255 | head -n $(( 64 * 64 * 20 )) | csv-to-bin ub ) \\\n";
    oss << "                      | cv-cat --input 'no-header;rows=64;cols=64;type=ub' 'count;accumulate=20,sliding;view=0;null'\n";
    oss << "        <options>\n";
    ...
    oss << "                 <spin-up>: default=1; see https://en.wikipedia.org/wiki/Moving_average#Exponential_moving_average\n";
    ...
    oss << "        blur=gaussian,<kernel_size>,<std_size>. Set std or kernel size to 0 to calculate based on other parameter.\n";
    oss << "        blur=bilateral,<neighbourhood_size>,<sigma_space>,<sigma_colour>; preserves edges\n";
    oss << "        blur=adaptive-bilateral: <kernel_size>,<sigma_space>,<sigma_colour_max>;\n";
    oss << "                                 preserve edges, automatically calculate sigma_colour (and cap to sigma_colour_max)\n";
    ...
    oss << "                                                     generates a mask with bright lines representing the edges on\n";
    oss << "                                                     a black background, requires single-channel 8-bit input image\n";
    oss << "            threshold1, threshold2: the smaller value is used for edge linking, the larger value is used to find\n";
    oss << "                                    initial segments of strong edges\n";
    ...
    oss << "        <how>\n";
    oss << "            <from>,<to>: convert from colour space to new colour space (BGR, RGB, HSV, Lab, XYZ, Bayer**, GRAY)\n";
    oss << "                         (if you need more just ask), see opencv cvtColor() for more; eg: BGR,GRAY or CV_BGR2GRAY\n";
    ...
    oss << "                                                          (count from zero)\n";
    oss << "                                                          if width or height of input image is not divisible by\n";
    oss << "                                                          the corresponding count, the image will be clipped\n";
    ...
    oss << "            \"crop-tile=2,5\": similar to tile operation; crop all tiles from the image of 2x5 tiles;\n";
    oss << "                             output image will be composed of 2*5=10 tiles\n";
    ...
    oss << "        examples: cv-cat --file image.jpg \\\n";
    oss << "                      \"split;crop-tile=2,5,0,0,1,3;convert-to=f,0.0039;fft;fft=inverse,magnitude;view;null\"\n";
    oss << "                  cv-cat --file image.jpg \\\n";
    ...
    oss << "              default: read a single column of floating point values (with row counter starting from zero as key)\n";
    oss << "         <permissive>: if present, integer values are simply copied to the output unless they are in the map\n";
    ...
    oss << "    overlay=<image_file>[,x,y]: overlay image_file on top of current stream at optional x,y location\n";
    oss << "                                overlay image should have alpha channel\n";
    ...
    oss << "                                       if width or height of input image is not divisible by the corresponding\n";
    oss << "                                       count the input image will be clipped\n";
    oss << "                                       horizontal: if present, input tiles are considered stacked horizontally\n";
    oss << "                                       (by default, vertical stacking is used)\n";
    ...
    oss << "            where 'a' is high quadbit of byte 0, 'b' is low quadbit of byte 0, 'c' is high quadbit of byte 1, etc...\n";
    oss << "            and '0' means quadbit zero\n";
    oss << "    unpack12: convert from 12-bit packed (2 pixels in 3 bytes) to 16UC1; use before other filters\n";
    oss << "              equivalent to unpack=12,abc0,efd0\n";
    oss << "    untile=<ncols>,<nrows>[,horizontal]: inverse to tile; input image used as a strip of <ncols>*<nrows> tiles,\n";
    oss << "                                         output image will be composed of <nrows> of tiles with <ncols> tiles per row\n";
    oss << "                                         if width or height of input image is not divisible by the corresponding\n";
    oss << "                                         count the input image will be clipped\n";
    oss << "                                         horizontal: if present, input tiles are considered stacked horizontally\n";
    oss << "                                         (by default, vertical stacking is used)\n";
    oss << "                                         example:\n";
    oss << "                                             $ ( yes 255 | head -n $(( 64 * 64 * 20 )) | csv-to-bin ub ) \\\n";
    oss << "                                                   | cv-cat --input 'no-header;rows=64;cols=64;type=ub' 'count' \\\n";
    oss << "                                                   | cv-cat --input 'no-header;rows=1280;cols=64;type=ub' \\\n";
    oss << "                                                                    'encode=png' > original.png\n";
    ...
    oss << "        log=<dirname>,size:<number of frames>: write images to files in a given directory, each file\n";
    oss << "                                               (except possibly the last one) containing <number of frames> frames\n";
    oss << "        log=<dirname>,period:<seconds>: write images to files in a given directory, each file containing frames\n";
    oss << "                                        for a given period of time\n";
    oss << "                                        e.g. for log=tmp,period:1.5 each file will contain 1.5 seconds of images\n";
    oss << "        log=<options>,index: write binary index file, with format t,uw,ul and fields:\n";
    oss << "                             timestamp, file number, and offset of each frame\n";
    oss << "    save=<filename>[,<quality>][,index][,no-header]: write images to files in the specified format.\n";
    oss << "        <filename>: <base>.<format>\n";
    oss << "        <format>: bin|jpg|ppm|png|tiff... anything that opencv imwrite can take\n";
    oss << "                  or 'bin' to write image as binary in cv-cat format\n";
    oss << "        <quality>: for jpg files, compression quality from 0 (smallest) to 100 (best)\n";
    oss << "        index: if present, for each image, files will be named as: <base>.<index>.<extension>,\n";
    oss << "               e.g: my-file.0.png, my-file.1.png, etc\n";
    oss << "\n";
    ...
    oss << "    merge=<n>: split an image into n horizontal bands of equal height and merge them into an n-channel image\n";
    oss << "               (the number of rows must be a multiple of n); default <n>: 3\n";
    oss << "    split: split n-channel image into a nx1 grey-scale image\n";
    oss << "    shuffle=<list>; re-shuffle input channels, e.g., shuffle=r,b,g - swap channels 1 and 2;\n";
    oss << "                    channels are described by symbolic names 'r', 'g', 'b', and 'a',\n";
    oss << "                    where 'r' is always channel[0], 'b' is channel[1], etc.;\n";
    oss << "                    if a field is left empty, the corresponding channel is copied verbatim from the input\n";
    oss << "                    more examples:\n";
    oss << "                        shuffle=,b      - drop channel b, leave 2 channels\n";
    oss << "                        shuffle=,       - drop channel b\n";
    oss << "                        shuffle=r,g,r,g - duplicate r and g, drop b\n";
    oss << "\n";
    oss << "operations on \"forked\" image stream:\n";
    oss << "    take input image, apply some filters to it, then apply the operation between the input and resulting image\n";
    oss << "    'double-forked' operations are not supported, e.g., you cannot use mask inside an arithmetic operation\n";
    oss << "    or vise versa\n";
    oss << "    usage: <operation>=<filters>\n";
    oss << "        <filters>: any sequence of cv-cat filters that outputs a single-channel image of the same dimensions\n";
    oss << "                   as the images on stdin\n";
    ...
    oss << "        maximum=<filters>: forked image is pixelwise maximum to the input image, see cv::max()\n";
    oss << "        minimum=<filters>: forked image is pixelwise minimum to the input image, see cv::min()\n";
    ...
    oss << "\n";
    oss << "    it is the users responsibility to convert data to integer format before applying bitwise combinations\n";
    oss << "\n";
    oss << "operations on subsets of columns, rows, or channels:\n";
    oss << "    bands-to-cols=x,w[,x,w][,method:<method-name>,output-depth:<depth>];\n";
    oss << "        take a number of columns (bands) from the input, process together by method,\n";
    oss << "        write into the output, one band (a range of columns) reduced into one column;\n";
    oss << "        supported methods: average (default), sum, min, max; the sum method\n";
    oss << "        requires explicit output-depth parameter (one of i,f,d or CV_32S,CV_32F,CV_64F) if the input has low depth\n";
    oss << "        examples:\n";
    oss << "            \"bands-to-cols=12,23,50,30,45,60,100,1,method:average\"\n";
    oss << "                output an image of 4 columns containing the average of columns 12-34 (i.e., 12 + 23 - 1),\n";
    oss << "                50-79, 45-104 (overlap is OK), and 100 (width is 1) from the original image\n";
    oss << "            \"bands-to-cols=12,23,50,30,45,60,100,1,method:sum,output-depth:d\"\n";
    oss << "                same bands but output an image of 4 columns containing the sum of the columns data from the\n";
    oss << "                original image; use CV_64F depth (double) as the output format\n";
    oss << "    bands-to-rows=x,w[,x,w][,method:<method-name>,output-depth:<depth>];\n";
    oss << "        same as bands-to-cols but operate on rows of input instead of columns\n";
    oss << "    channels-to-cols; opposite to cols-to-channels; unwrap all channels as columns\n";
    oss << "        example: \"channels-to-cols\" over a 3-channel image:\n";
    oss << "                 RGB channels of column 0 become columns 0 (single-channel), 1, and 2\n";
    oss << "                 RGB channels of column 1 become columns 3,4,5, and so on\n";
    oss << "    channels-to-rows; same as channels-to-cols but operates over rows\n";
    oss << "    cols-to-channels=i,j,k[,pad:value,repeat:step]; opposite to channels-to-cols;\n";
    oss << "        stores the listed columns as channels in the output file\n";
    oss << "        input shall be a single-channel stream; up to 4 channels are supported; if 1, 3, or 4 columns are specified\n";
    oss << "        the output would have 1, 3, or 4 channels respectively\n";
    oss << "        in case of 2 columns, a third empty (zero) channel is added; use the \"pad:value\" option to specify the\n";
    oss << "        fill value other then zero\n";
    oss << "        the repeat option applies the transformation periodically, first for the specified columns, then for columns\n";
    oss << "        incremented by one step, and so on; see the examples\n";
    oss << "        examples:\n";
    oss << "            \"cols-to-channels=6,4,pad:128\"\n";
    oss << "                put column 6 into the R channel, column 4 into the G channel, and fill the B channel with 128\n";
    oss << "            \"cols-to-channels=0,1,2,repeat:3\"\n";
    oss << "                store columns 0,1,2 as RGB channels of column 0 of the output file, then columns 3,4,5\n";
    oss << "                as RGB channels of column 1 of the output file, etc.; conversion stops when all input\n";
    oss << "                column indices exceed the image width; if one of the input columns exceed the image width\n";
    oss << "                the respective output channel is filled with zeros (or the padding value)\n";
    oss << "    crop-cols=<x>,<w>[,<x>,<w>,...]: output an image consisting of (multiple) columns starting at x with width w\n";
    oss << "        examples:\n";
    oss << "            \"crop-cols=2,10,12,10\"; output an image of width 20 taking 2 width-10 columns starting at 2 and 12\n";
    ...
    oss << "        examples: \"crop-rows=5,10,25,5\"; output an image of height 15 taking 2 row blocks starting at 5 and 25\n";
    oss << "                                           and with heights 10 and 5, respectively\n";
    ...
    oss << "            erode=rectangle,<size/x>,<size/y>[,<anchor/x>,<anchor/y>][,iterations];\n";
    oss << "                apply erosion with a rectangular structuring element\n";
    oss << "            erode=square,<size/x>,[<anchor/x>],[<anchor/y>],[iterations];\n";
    oss << "                apply erosion with a square structuring element of custom size\n";
    oss << "            erode=ellipse,<size/x>,<size/y>[,<anchor/x>,<anchor/y>],[iterations];\n";
    oss << "                apply erosion with an elliptic structuring element\n";
    oss << "            erode=circle,<size/x>,[<anchor/x>],[<anchor/y>],[iterations];\n";
    oss << "                apply erosion with a circular structuring element\n";
    oss << "            erode=cross,<size/x>,<size/y>,[<anchor/x>],[<anchor/y>],[iterations];\n";
    oss << "                apply erosion with a circular structuring element\n";
    oss << "                note that the structuring element shall usually be symmetric, and therefore\n";
    oss << "                size/s,size/y shall be odd\n";
    oss << "                any of the parameters after shape name can be omitted (left as empty csv field) to use the defaults:\n";
    oss << "                    - size/x = 3:\n";
    oss << "                    - size/y = size/x:\n";
    ...
    oss << "                alternatively, if only 2 parameters are given for rectangle, ellipse, and cross, they are\n";
    oss << "                interpreted as size/x,size/y, with the anchor set to default; similarly, if a single parameter\n";
    oss << "                is given for square and circle, it is used to set size with default anchor\n";
    oss << "\n";
    ...
    oss << "                  \"open=circle,7\"; apply opening with a radius 7 circle anchored at the center (single parameter)\n";
    oss << "                  \"open=rectangle,7,3\"; apply opening with a 7x3 rectangle anchored at the center (two parameters)\n";
    oss << "\n";
    oss << "multiple-channel operations:\n";
    oss << "    linear-combination=<a1>r + <a2>g + ... + <ac>:\n";
    oss << "        output a grey-scale image that is a linear combination of input channels with given coefficients\n";
    oss << "        and optional offset\n";
    oss << "        example: \"linear-combination=-r+2g-b\", highlights the green channel\n";
    oss << "        naming conventions are the same as for the ratio operation; use '--help filters::linear-combination'\n";
    oss << "        for more examples and a detailed syntax explanation\n";
    oss << "    linear-combination=expr0,expr1...:\n";
    oss << "        output multi-channel image where each output channel is calculated as a linear combination of input channels\n";
    oss << "        according to the provided comma-separated expressions (expr0 goes to channel 0, expr1 to channel 1, etc.);\n";
    oss << "        if any of the expressions is omitted (empty), it is interpreted as a pass-through of the respective channel\n";
    oss << "    ratio=(<a1>r + <a2>g + ... + <ac>)/(<b1>r + <b2>g + ... + <bc>):\n";
    oss << "        output a grey-scale image that is a ratio of linear combinations of input channels with given coefficients\n";
    oss << "        and offsets;\n";
    oss << "        see below for examples, use '--help filters::ratio' for the detailed explanation of the syntax and examples\n";
    oss << "        the naming convention does not depend on the actual image channels: 'r' in the ratio expression is always\n";
    oss << "        interpreted as channel[0], 'g' as channel[1], etc.; in particular, grey-scaled images have a single channel\n";
    oss << "        that shall be referred to as 'r', e.g., ratio=r / ( r + 1 )\n";
    oss << "    ratio=expr0,expr1...:\n";
    oss << "        output a multi-channel image where each output channel is calculated as a ratio of linear combinations\n";
    oss << "        of input channels given by the provided comma-separated expressions (expr0 goes to channel 0,\n";
    oss << "        expr1 to channel 1, etc.); if any of the expressions is empty (omitted), it is interpreted as a pass-through\n";
    oss << "        (see examples)\n";
    oss << "\n";
    oss << "        examples:\n";
    oss << "            \"ratio=( r + g + b ) / ( 1 + a )\"; output a grey-scale image equal to the sum of the first 3 channels\n";
    oss << "                                               divided by the offset 4th channel\n";
    oss << "            \"ratio=( r - b ) / ( r + b )\";  output normalized difference of channels 'r' and 'g'\n";
    oss << "            \"ratio=,( r - b ) / ( r + b )\"; output a 2-channel image, output channel 0 (r) is copied from the\n";
    oss << "                                            input as is, while output channel 1 (g) contains normalized difference\n";
    oss << "                                            of channels 'r' and 'g'\n";
    oss << "\n";
    oss << "    output of the ratio and linear-combination operations has floating point (CV_32F) precision unless the input\n";
    oss << "    is already in doubles (if so, precision is unchanged)\n";
    oss << "\n";
    oss << "cv::Mat image operations\n";
    oss << "    simple-blob[=<parameters>]: wraps cv::SimpleBlobDetector, outputs key points timestamped by image timestamp\n";
    oss << "        <parameters>\n";
