...
static std::string usage_impl_()
{
    ...
    oss << "\n";
    oss << "    OpenCV version: " << CV_VERSION << "\n";
    oss << "\n";
    oss << "    cv::Mat image filters usage (';'-separated):\n";
    oss << "        accumulate=<n>[,<how>][,<options>]: accumulate the last n images and concatenate them vertically (useful for slit-scan and spectral cameras like pika2)\n";
    oss << "            example: cat slit-scan.bin | cv-cat \"accumulate=400;view;null\"\n";
    oss << "            <how>\n";
    oss << "                sliding: sliding window\n";
    oss << "                fixed: input image location in the output image is defined by its number modulo <n>\n";
    oss << "                default: sliding\n";
    oss << "                example: run the command line below; press white space key to see image accumulating; try it with fixed or fixed-inverse\n";
    oss << "                    > ( yes 255 | head -n $(( 64 * 64 * 20 )) | csv-to-bin ub ) | cv-cat --input 'no-header;rows=64;cols=64;type=ub' 'count;accumulate=20,sliding;view=0;null'\n";
    oss << "            <options>\n";
    oss << "                reverse: e.g. fixed,reverse: same as fixed, but in the opposite order\n";
    oss << "                time:<how>; how to timestamp accumulated image\n";
    oss << "                    first: by first input frame\n";
    oss << "                    last: by last input frame\n";
    oss << "                    mean: by mean input timestamp value across accumulated frame (todo)\n";
    oss << "                    median: by middle input timestamp of accumulated frame (todo)\n";
    oss << "        accumulated=<operation>: apply a pixel-wise operation to the input images\n";
    oss << "            <operation>\n";
    oss << "                 average: pixelwise average using all images from the beginning of the stream\n";
    oss << "                 ema,<alpha>[,<spin_up>]: pixelwise exponential moving average\n";
    oss << "                     <alpha>: range: between 0 and 1.0, larger value will retain more historical data\n";
    oss << "                     <spin-up>: default = 1; for details see https://en.wikipedia.org/wiki/Moving_average#Exponential_moving_average\n";
    oss << "                     formula: ema += ( new_pixel_value - ema ) * <alpha>\n";
    oss << "                 min: pixelwise image minimum\n";
    ...
    oss << "            blur=median,<kernel_size>\n";
    oss << "            blur=gaussian,<kernel_size>,<std_size>. Set either std or kernel size to 0 to calculate based on other parameter.\n";
    oss << "            blur=bilateral,<neighbourhood_size>,<sigma_space>,<sigma_colour>; preserves edges\n";
    oss << "            blur=adaptive-bilateral: <kernel_size>,<sigma_space>,<sigma_colour_max>; preserve edges, automatically calculate sigma_colour (and cap to sigma_colour_max)\n";
    oss << "        brightness,scale=<scale>[,<offset>]: output=(scale*input)+offset; default offset=0\n";
    oss << "        canny=<threshold1>,<threshold2>[,<kernel_size>]: finds edges using the Canny86 algorithm (see cv::Canny)\n";
    oss << "                                                         generates a mask with bright lines representing the edges on a black background, requires single-channel 8-bit input image\n";
    oss << "                threshold1, threshold2: the smaller value is used for edge linking, the larger value is used to find initial segments of strong edges\n";
    oss << "        clahe=<clip_limit>,<tile_size_x>[,<tile_size_y>]: CLAHE, contrast limited adaptive histogram equalization (see opencv documentation for more), e.g. try clahe=2.0,8,8\n";
    ...
    oss << filters::contraharmonic< boost::posix_time::ptime >::usage(8) << "\n";
    oss << "        count: write frame number on images\n";
    oss << "        convert-to,convert_to=<type>[,<scale>[,<offset>]]: convert to given type; should be the same number of channels; see opencv cvtColor() for details; values will not overflow\n";
    oss << "        convert-color=<how>; convert from colour space to new colour space\n";
    oss << "            <how>\n";
    oss << "                <from>,<to>: convert from colour space to new colour space (BGR, RGB, HSV, Lab, XYZ, Bayer**, GRAY supported; if you need more just ask, see opencv cvtColor() for more); eg: BGR,GRAY or CV_BGR2GRAY\n";
    ...
    oss << "                     quick and dirty for now, but lets use dozens of conversions\n";
    oss << filters::convolution< boost::posix_time::ptime >::usage(8) << "\n";
    oss << "        crop=[<x>,<y>],<width>,<height>: crop the portion of the image starting at x,y with size width x height\n";
    oss << "        crop-tile=<ncols>,<nrows>[,<i>,<j>,...[,horizontal]]: divide the image into a grid of tiles (ncols-by-nrows), and output an image made of the croped tiles defined by i,j (count from zero)\n";
    oss << "                                                              if width or height of input image is not divisible by the corresponding count, the image will be clipped\n";
    oss << "            horizontal: if present, tiles will be stacked horizontally (by default, vertical stacking is used)\n";
    oss << "            examples\n";
    oss << "                \"crop-tile=2,5,1,0,2,3\": crop 2 tiles of the image 2x5 tiles: tile at 1,0 and at 2,3\n";
    oss << "                \"crop-tile=2,5\": similar to tile operation; crop all tiles from the image of 2x5 tiles; output image will be composed of 2*5=10 tiles\n";
    oss << "                \"crop-tile=2,5,1,0,2,3,horizontal\": crop 2 tiles out of image split into 2x5 tiles: tile at 1,0 and at 2,3; arrange tiles horizontally in the output image\n";
    oss << "            deprecated: old syntax <i>,<j>,<ncols>,<nrows> is used for one tile if i < ncols and j < ncols\n";
    oss << "        encode=<format>[,<quality>]: encode images to the specified format. <format>: jpg|ppm|png|tiff..., make sure to use --no-header\n";
    ...
    oss << "                     real: output real part only\n";
    oss << "                     magnitude: output magnitude only\n";
    oss << "            examples: cv-cat --file image.jpg \"split;crop-tile=2,5,0,0,1,3;convert-to=f,0.0039;fft;fft=inverse,magnitude;view;null\"\n";
    oss << "                      cv-cat --file image.jpg \"split;crop-tile=2,5,0,0,1,3;convert-to=f,0.0039;fft=magnitude;convert-to=f,40000;view;null\"\n";
    oss << "        flip: flip vertically\n";
    oss << "        flop: flip horizontally\n";
    oss << filters::gamma<>::usage( 8 ) << "\n";
    oss << "        grab=<format>[,<quality>]: write an image to file with timestamp as name in the specified format. <format>: jpg|ppm|png|tiff..., if no timestamp, system time is used\n";
    oss << "                                   <quality>: for jpg files, compression quality from 0 (smallest) to 100 (best)\n";
    oss << filters::hard_edge< boost::posix_time::ptime >::usage( 8 ) << "\n";
    oss << "        head=<n>: output <n> frames and exit\n";
    oss << "        inrange=<lower>,<upper>: a band filter on r,g,b or greyscale image; for rgb: <lower>::=<r>,<g>,<b>; <upper>::=<r>,<g>,<b>; see cv::inRange() for detail\n";
    oss << "        invert: invert image (to negative)\n";
    oss << "        kmeans=<k>[,<params>]: perform k-means clustering on image and replace each pixel with the mean of its cluster\n";
    oss << "        magnitude: calculate magnitude for a 2-channel image; see cv::magnitude() for details\n";
    oss << "        map=<map file>[&<csv options>][&permissive]: map integer values to floating point values read from the map file\n";
    oss << "             <csv options>: usual csv options for map file, but &-separated (running out of separator characters)\n";
    oss << "                  fields: key,value; default: value\n";
    oss << "                  default: read a single column of floating point values (with the row counter starting from zero used as key)\n";
    oss << "             <permissive>: if present, integer values in the input are simply copied to the output unless they are in the map\n";
    oss << "                  default: filter fails with an error message if it encounters an integer value which is not in the map\n";
    oss << "             example: \"map=map.bin&fields=,key,value&binary=2ui,d\"\n";
    oss << "        normalize=<how>[,<norm_type>]: normalize image and scale to 0 to 1 float (or double if input is CV_64F)\n";
    oss << "            <how>\n";
    oss << "                normalize=max: normalize each pixel channel by its max value\n";
    oss << "                normalize=sum: normalize each pixel channel by the sum of all channels\n";
    oss << "                normalize=all: normalize each pixel by max of all channels (cv::normalize())\n";
    oss << "            <norm_type>\n";
    oss << "                inf: cv::NORM_INF\n";
    oss << "                l1: cv::NORM_L1\n";
    oss << "                l2: cv::NORM_L2\n";
    oss << "                default: inf (for backward compatibility)\n";
    oss << "        null: same as linux /dev/null (since windows does not have it)\n";
    oss << "        overlay=<image_file>[,x,y]: overlay image_file on top of current stream at optional x,y location; overlay image should have alpha channel\n";
    oss << "        pack=<bits>,<format>: pack pixel data in specified bits, example: pack=12,cd,hb,fg\n";
    oss << "            <bits>: number of bits, currently only 12-bit packing from 16-bit is supported (pack 2 pixels into 3 bytes)\n";
    oss << "            <format>: output pixel formats in quadbits\n";
    oss << "                where 'a' is high quadbit of byte 0, 'b' is low quadbit of byte 0, 'c' is high quadbit of byte 1, etc... and '0' means quadbit zero\n";
    oss << filters::partitions::partition< boost::posix_time::ptime >::usage( 8 ) << "\n";
    oss << filters::partitions::reduce< boost::posix_time::ptime >::usage( 8 ) << "\n";
    oss << "        pow,power=<value>; each image channel power, currently plain wrapper of opencv pow(), thus may be slow; todo? parallelize and/or implement mapping with interpolation\n";
    oss << "        remap=<map-filename>[,<interpolation>]: remap, input image dimensions expected to match map dimentions; see cv::remap() for details\n";
    ...
    oss << "            <interpolation>: nearest, linear, area, cubic, lanczos4; default: linear\n";
    oss << "                             in format <width>,<height>,<interpolation> corresponding numeric values can be used: " << cv::INTER_NEAREST << ", " << cv::INTER_LINEAR << ", " << cv::INTER_AREA << ", " << cv::INTER_CUBIC << ", " << cv::INTER_LANCZOS4 << "\n";
    oss << "            examples\n";
    ...
    oss << "        remove-mean=<kernel_size>,<ratio>: simple high-pass filter removing <ratio> times the mean component on <kernel_size> scale\n";
    oss << "        remove-speckles=<x>,<y>: simple speckle remover: if kernel of size <x>,<y> and single solid colour fully covers speckle of another color, fill the speckle with surrounding color\n";
    oss << "        rotate90[=n]: rotate image 90 degrees clockwise n times (default: 1); sign denotes direction (convenience wrapper around { tranpose, flip, flop })\n";
    //oss << "        warp=<how>: todo: warp image\n";
    //oss << "        unwarp=<how>: todo: unwarp image\n";
    //oss << "            <how>: todo\n";
    oss << filters::pad::pad< boost::posix_time::ptime >::usage(8) << "\n";
    oss << filters::text< boost::posix_time::ptime >::usage( 8 ) << "\n";
    oss << "        threshold=<threshold|otsu>[,<maxval>[,<type>]]: threshold image; same semantics as cv::threshold()\n";
    oss << "            <threshold|otsu>: threshold value; if 'otsu' then the optimum threshold value using the Otsu's algorithm is used (only for 8-bit images)\n";
    oss << "            <maxval>: maximum value to use with the binary and binary_inv thresholding types (default:255)\n";
    oss << "            <type>: binary, binary_inv, trunc, tozero, tozero_inv (default:binary)\n";
    oss << "        tile=<ncols>,<nrows>[,horizontal]: input image split into <ncols>*<nrows> tiles; also see untile\n";
    oss << "                                           if width or height of input image is not divisible by the corresponding count, the input image will be clipped\n";
    oss << "                                           horizontal: if present, input tiles are considered stacked horizontally (by default, vertical stacking is used)\n";
    oss << "        timestamp: write timestamp on images\n";
    oss << "        transpose: transpose the image (swap rows and columns)\n";
    oss << "        undistort=<how>: undistort\n";
    oss << "            <how>\n";
    oss << "                <undistort_map_filename>, e.g. map.bin\n";
    oss << "                <pinhole_config_filename>[:<path>], e.g. config.json:camera/pinhole\n";
    oss << "        unpack=<bits>,<format>: unpack compressed pixel formats, example: unpack=12,cba0,fed0\n";
    oss << "            <bits>: number of bits, currently only 12 is supported\n";
    oss << "            <format>: output pixel formats in quadbits\n";
    oss << "                where 'a' is high quadbit of byte 0, 'b' is low quadbit of byte 0, 'c' is high quadbit of byte 1, etc... and '0' means quadbit zero\n";
    oss << "        unpack12: convert from 12-bit packed (2 pixels in 3 bytes) to 16UC1; use before other filters; equivalent to unpack=12,abc0,efd0\n";
    oss << "        untile=<ncols>,<nrows>[,horizontal]: inverse to tile; input image used as a strip of <ncols>*<nrows> tiles, output image will be be composed by <nrows> of tiles with <ncols> tiles per row\n";
    oss << "                                             if width or height of input image is not divisible by the corresponding count, the input image will be clipped\n";
    oss << "                                             horizontal: if present, input tiles are considered stacked horizontally (by default, vertical stacking is used)\n";
    oss << "                                             example\n";
    oss << "                                                 > ( yes 255 | head -n $(( 64 * 64 * 20 )) | csv-to-bin ub ) | cv-cat --input 'no-header;rows=64;cols=64;type=ub' 'count' | cv-cat --input 'no-header;rows=1280;cols=64;type=ub' 'encode=png' > original.png\n";
    oss << "                                                 > eog original.png\n";
    oss << "                                                 > ( yes 255 | head -n $(( 64 * 64 * 20 )) | csv-to-bin ub ) | cv-cat --input 'no-header;rows=64;cols=64;type=ub' 'count' | cv-cat --input 'no-header;rows=1280;cols=64;type=ub' 'untile=5,4;encode=png' > untiled.png\n";
    oss << "                                                 > eog untiled.png\n";
    ...
    oss << "                                press numerical '0' to '9' to add the id (0-9) to the file name: <timestamp>.<id>.<suffix>\n";
    oss << "                                press any other key to show the next frame\n";
    oss << "                                <wait-interval>: a hack for now; milliseconds to wait for image display or key press (-1 waits indefinitely); default 1\n";
    ...
    oss << "            attention! it seems that lately using cv::imshow() in multithreaded context has been broken in opencv or in underlying x window stuff\n";
    ...
    oss << filters::file< boost::posix_time::ptime >::usage( 8 ) << "\n";
    oss << "        load=<filename>: load image from file instead of taking an image on stdin; the main meaningful use would be in association with 'forked' image processing\n";
    oss << "            supported file types by filename extension:\n";
    oss << "                - .bin or <no filename extension>: file is in cv-cat binary format: <t>,<rows>,<cols>,<type>,<image data>\n";
    oss << "                - otherwise whatever cv::imread and cv::VideoCapture support\n";
    oss << "        log=<options>: write images to files\n";
    oss << "            log=<filename>: write images to a single file\n";
    oss << "            log=<dirname>,size:<number of frames>: write images to files in a given directory, each file (except possibly the last one) containing <number of frames> frames\n";
    ...
    oss << "        merge=<n>: split an image into n horizontal bands of equal height and merge them into an n-channel image (the number of rows must be a multiple of n); default <n>: 3\n";
    oss << "        split: split n-channel image into a nx1 grey-scale image\n";
    oss << "        shuffle=<list>; re-shuffle input channels, e.g., shuffle=r,b,g - swap channels 1 and 2; channels are described by symbolic names 'r', 'g', 'b', and 'a',\n";
    oss << "            where 'r' is always channel[0], 'b' is channel[1], etc.; if a field is left empty, the corresponding channel is copied verbatim from the input\n";
    oss << "            more examples: shuffle=,b - drop channel b, leave 2 channels; shuffle=, - drop channel b; shuffle=r,g,r,g - duplicate r and g, drop b\n";
    oss << "\n";
    oss << "    operations on \"forked\" image stream:\n";
    oss << "        semantics: take input image, apply some filters to it, then apply the operation between the input and resulting image\n";
    oss << "        'double-forked' operations are not supported, e.g., you cannot use mask inside an arithmetic operation or vise versa\n";
    oss << "        usage: <operation>=<filters>\n";
    oss << "            <filters>: any sequence of cv-cat filters that outputs a single-channel image of the same dimensions as the images on stdin\n";
    oss << "                the separator between the filters is '|'\n";
    oss << "                the equal sign for a filter is ':'\n";
    oss << "            e.g: cat images.bin | cv-mat subtract=\"average|threshold:0.5\"\n";
    oss << "\n";
    oss << "        arithmetic operations\n";
    oss << "            add=<filters>: forked image is pixelwise added to the input image, see cv::add()\n";
    oss << "            divide=<filters>: forked image is pixelwise divided to the input image, see cv::divide()\n";
    oss << "            multiply=<filters>: forked image is pixelwise multiplied to the input image, see cv::multiply()\n";
    oss << "            subtract=<filters>: forked image is pixelwise subtract to the input image, see cv::subtract()\n";
    oss << "            absdiff=<filters>: forked image is pixelwise absolute difference between images, see cv::absdiff()\n";
    oss << "                examples:\n";
    oss << "                    multiply operation with accumulated and threshold sub filters\n";
    oss << "                        cat images.bin | cv-cat \"multiply=accumulated:average,5|threshold:0.5,1.0\" >results.bin\n";
    oss << "                    scaling input images by a mask file, input type:  3ub, mask type: 3f\n";
    oss << "                        cat images.bin | cv-cat \"multiply=load:mask.bin\" >results.bin\n";
    oss << "            forked arithmetic operations can use bitwise combinations of filters\n";
    oss << "\n";
    oss << "        bitwise operations on filters:\n";
    oss << "            bitwise=<filter1> and <filter2>: apply bitwise 'and' of <filter1> and <filter2> outputs\n";
    oss << "            bitwise=<filter1> or <filter2>: apply bitwise 'or'\n";
    oss << "            bitwise=<filter1> xor <filter2>: apply bitwise 'xor'\n";
    oss << "            bitwise=not <filter1>: apply bitwise 'not' (complement, tilde in C++)\n";
    oss << "            bitwise=(( not <filter1> ) and <filter2>) or <filter3>: apply the given logical expression of 3 masks\n";
    oss << "            note: standard precedence rules apply; use brackets to explicitly denote precedence\n";
    oss << "\n";
    oss << "        mask=<filters>: apply mask to image (see cv::copyTo for details)\n";
    oss << "            examples\n";
    oss << "                apply a constant mask from a file\n";
    oss << "                    cat images.bin | cv-cat 'mask=load:mask.bin' > masked.bin\n";
    oss << "                    cat images.bin | cv-cat 'mask=load:mask.png' > masked.bin\n";
    oss << "                extract pixels brighter than 100\n";
    oss << "                    cat images.bin | cv-cat 'mask=convert-color:BGR,GRAY|threshold=otsu,100' > masked.bin\n";
    oss << "            masks can use bitwise operations, e.g.\n";
    oss << "                cv-cat \"mask=ratio:(r + b - g)/( 1 + r + b )|convert-to:ub|threshold:4 xor ratio:2./(1.5e1 - g + r)|convert-to:ub|threshold:5\"\n";
    oss << "\n";
    oss << "        tee=<filters>: run a forked pipeline; pass the image in the main pipeline unchanged\n";
    oss << "            examples\n";
    oss << "                view modified image\n";
    oss << "                    cat images.bin | cv-cat 'tee=resize:0.25|timestamp|view;...'\n";
    oss << "\n";
    oss << "        it is the user responsibility to convert data to integer format before applying bitwise combinations\n";
    oss << "\n";
    oss << "    operations on subsets of columns, rows, or channels\n";
    oss << "        bands-to-cols=x,w[,x,w][,method:<method-name>,output-depth:<depth>]; take a number of columns (bands) from the input, process together by method,\n";
    oss << "            write into the output, one band (a range of columns) reduced into one column; supported methods: average (default), sum, min, max; the sum method\n";
    oss << "            requires the explicit output-depth parameter (one of i,f,d or CV_32S, CV_32F, CV_64F) if the input has low depth\n";
    oss << "            examples: \"bands-to-cols=12,23,50,30,45,60,100,1,method:average\"; output an image of 4 columns containing the average\n";
    oss << "                          of columns 12-34 (i.e., 12 + 23 - 1), 50-79, 45-104 (overlap is OK), and 100 (width is 1) from the original image\n";
    oss << "                      \"bands-to-cols=12,23,50,30,45,60,100,1,method:sum,output-depth:d\"; same bands but output an image of 4 columns containing the sum\n";
    oss << "                          of the columns data from the original image; use CV_64F depth (double) as the output format\n";
    oss << "        bands-to-rows=x,w[,x,w][,method:<method-name>,output-depth:<depth>]; same as bands-to-cols but operate on rows of input instead of columns\n";
    oss << "\n";
    oss << "        channels-to-cols; opposite to cols-to-channels; unwrap all channels as columns\n";
    oss << "            example: \"channels-to-cols\" over a 3-channel image: RGB channels of column 0 become columns 0 (single-channel), 1, and 2, RGB channels\n";
    oss << "            of column 1 become columns 3,4,5, and so on\n";
    oss << "        channels-to-rows; same as channels-to-cols but operates over rows\n";
    oss << "\n";
    oss << "        cols-to-channels=i,j,k[,pad:value,repeat:step]; opposite to channels-to-cols; stores the listed columns as channels in the output file\n";
    oss << "            input shall be a single-channel stream; up to 4 channels are supported; if 1, 3, or 4 columns are specified, the output would have 1, 3, or 4 channels respectively\n";
    oss << "            in case of 2 columns, a third empty (zero) channel is added; use the \"pad:value\" option to specify the fill value other then zero\n";
    oss << "            the repeat option applies the transformation periodically, first for the specified columns, then for columns incremented by one step, and so on; see the examples\n";
    oss << "            examples: \"cols-to-channels=6,4,pad:128\"; put column 6 into the R channel, column 4 into the G channel, and fill the B channel with 128\n";
    oss << "                      \"cols-to-channels=0,1,2,repeat:3\"; store columns 0,1,2 as RGB channels of column 0 of the output file, then columns 3,4,5 as RGB\n";
    oss << "                      channels of column 1 of the output file, etc.; conversion stops when all input column indices exceed the image width\n";
    oss << "                      if one of the input columns exceed the image width, the respective output channel is filled with zeros (or the padding value)\n";
    oss << "\n";
    oss << "        crop-cols=<x>,<w>[,<x>,<w>,...]: output an image consisting of (multiple) columns starting at x with width w\n";
    oss << "            examples: \"crop-cols=2,10,12,10\"; output an image of width 20 taking 2 width-10 columns starting at 2 and 12\n";
    oss << "                      \"crop-cols=2,1,12,10\"; width is 1, the output image has width 11\n";
    oss << "        crop-rows=<y>,<h>[,<y>,<h>,...]: output an image consisting of (multiple) row blocks starting at y with height h\n";
    oss << "            examples: \"crop-rows=5,10,25,5\"; output an image of height 15 taking 2 row blocks starting at 5 and 25 and with heights 10 and 5, respectively\n";
    ...
    oss << "        circle=<x>,<y>,<radius>[,<r>,<g>,<b>,<thickness>,<line_type>,<shift>]: draw circle; see cv::circle for details on parameters and defaults\n";
    oss << "        rectangle,box=<x>,<y>,<x>,<y>[,<r>,<g>,<b>,<thickness>,<line_type>,<shift>]: draw rectangle; see cv::rectangle for details on parameters and defaults\n";
    oss << "\n";
    oss << "    morphology operations\n";
    oss << "        opencv morphology operations\n";
    oss << "            blackhat=[<parameters>]; apply black-hat operation with the given parameters\n";
    oss << "            close=[<parameters>], closing=[<parameters>]; apply closing with the given parameters\n";
    oss << "            dilate=[<parameters>], dilation=[<parameters>]; apply dilation with the given parameters\n";
    oss << "            erode=[<parameters>], erosion=[<parameters>]; apply erosion with the given parameters\n";
    oss << "            gradient=[<parameters>]; apply morphological gradient with the given parameters\n";
    oss << "            open=[<parameters>], opening=[<parameters>]; apply opening with the given parameters\n";
    oss << "            tophat=[<parameters>]; apply top-hat operation with the given parameters\n";
    oss << "            skeleton=[<parameters>], thinning=[<parameters>]; apply skeletonization (thinning) with the given parameters\n";
    oss << "        custom morphology operations\n";
    oss << "            advance=[<parameters>][,background:<value>]; similar to dilate, but dilates only background pixels to\n";
    oss << "                                                         the nearest non-background value in kernel\n";
    oss << "                                                         only shape <parameters> supported\n";
    oss << "                                                         background:<value>: background pixel value; default: 0\n";
    oss << "            retreat=[<parameters>][,background:<value>][,force]; similar to erode, but erodes only non-background pixels,\n";
    oss << "                                                         if there is a background pixel in the kernel\n";
    oss << "                                                         only shape <parameters> supported\n";
    oss << "                                                         background:<value>: background pixel value; default: 0\n";
    oss << "                                                         force: drops requirement for background pixel in the kernel\n";
    oss << "                                                         resulting in borders (of background) around edges\n";
    oss << "            meet=[<parameters>]; similar to dilate, but decides on the pixel value based on the majority of\n";
    oss << "                                 pixels in the kernel weighted by distance to anchor\n";
    oss << "                                 only shape <parameters> supported\n";
    oss << "\n";
    oss << "            <parameters> for all the above operations have the same syntax; erode as an example is shown below:\n";
    oss << "                erode=rectangle,<size/x>,<size/y>[,<anchor/x>,<anchor/y>][,iterations]; apply erosion with a rectangular structuring element\n";
    oss << "                erode=square,<size/x>,[<anchor/x>],[<anchor/y>],[iterations]; apply erosion with a square structuring element of custom size\n";
    oss << "                erode=ellipse,<size/x>,<size/y>[,<anchor/x>,<anchor/y>],[iterations]; apply erosion with an elliptic structuring element\n";
    oss << "                erode=circle,<size/x>,[<anchor/x>],[<anchor/y>],[iterations]; apply erosion with a circular structuring element\n";
    oss << "                erode=cross,<size/x>,<size/y>,[<anchor/x>],[<anchor/y>],[iterations]; apply erosion with a circular structuring element\n";
    oss << "                    note that the structuring element shall usually be symmetric, and therefore, size/s,size/y shall be odd\n";
    oss << "                    any of the parameters after the shape name can be omitted (left as an empty csv field) to use the defaults:\n";
    oss << "                        - size/x = 3:\n";
    oss << "                        - size/y = size/x:\n";
    oss << "                        - anchor/x = center in x\n";
    oss << "                        - anchor/y = anchor/x\n";
    oss << "                        - iterations = 1\n";
    oss << "                    anchor value of -1 is interpreted as the center of the element\n";
    oss << "                    alternatively, if only 2 parameters are given for rectangle, ellipse, and cross, they are interpreted as size/x,\n";
    oss << "                    size/y, with the anchor set to default; similarly, if a single parameter is given for square and circle,\n";
    oss << "                    it is used to set size with default anchor\n";
    oss << "\n";
    oss << "            examples: \"erode=rectangle,5,3,,\"; apply erosion with a 5x3 rectangle anchored at the center\n";
    oss << "                      \"erode=rectangle,5,3,,,2\"; same as above but apply 2 iterations of erode\n";
    oss << "                      \"close\"; apply closing with a 3x3 square structuring element anchored at the center (default)\n";
    oss << "                      \"tophat=rectangle,11,,3,3\"; apply tophat with a 11x11 square and custom off-center anchor\n";
    oss << "                      \"dilate=cross,7,,,\"; apply dilation with a 7x7 cross anchored at the center\n";
    oss << "                      \"open=circle,7\"; apply opening with a radius 7 circle anchored at the center (note single parameter)\n";
    oss << "                      \"open=rectangle,7,3\"; apply opening with a 7x3 rectangle anchored at the center (note only two parameters)\n";
    oss << "\n";
    oss << "    multiple-channel operations\n";
    oss << "        linear-combination=<a1>r + <a2>g + ... + <ac>: output a grey-scale image that is a linear combination of input channels with given coefficients and optional offset\n";
    oss << "            example: \"linear-combination=-r+2g-b\", highlights the green channel\n";
    oss << "            naming conventions are the same as for the ratio operation; use '--help filters::linear-combination' for more examples and a detailed syntax explanation\n";
    oss << "        linear-combination=expr0,expr1...: output multi-channel image where each output channel is calculated as a linear combination of input channels\n";
    oss << "            according to the provided comma-separated expressions (expr0 goes to channel 0, expr1 to channel 1, etc.); if any of the expressions is omitted\n";
    oss << "            (empty), it is interpreted as a pass-through of the respective channel\n";
    oss << "        ratio=(<a1>r + <a2>g + ... + <ac>)/(<b1>r + <b2>g + ... + <bc>): output a grey-scale image that is a ratio of linear combinations of input channels\n";
    oss << "            with given coefficients and offsets; see below for examples, use '--help filters::ratio' for the detailed explanation of the syntax and examples\n";
    oss << "            the naming convention does not depend on the actual image channels: 'r' in the ratio expression is always interpreted as channel[0],\n";
    oss << "            'g' as channel[1], etc.; in particular, grey-scaled images have a single channel that shall be referred to as 'r', e.g., ratio=r / ( r + 1 )\n";
    oss << "        ratio=expr0,expr1...: output a multi-channel image where each output channel is calculated as a ratio of linear combinations of input channels\n";
    oss << "            given by the provided comma-separated expressions (expr0 goes to channel 0, expr1 to channel 1, etc.; if any of the expressions is empty\n";
    oss << "            (omitted), it is interpreted as a pass-through (see examples)\n";
    oss << "\n";
    oss << "            examples: \"ratio=( r + g + b ) / ( 1 + a )\"; output a grey-scale image equal to the sum of the first 3 channels\n";
    oss << "                          divided by the offset 4th channel\n";
    oss << "                      \"ratio=( r - b ) / ( r + b )\"; output normalized difference of channels 'r' and 'g'\n";
    oss << "                      \"ratio=,( r - b ) / ( r + b )\"; output a 2-channel image, output channel 0 (r) is copied from the input as is, while output channel 1 (g)\n";
    oss << "                          contains normalized difference of channels 'r' and 'g'\n";
    oss << "\n";
    oss << "        output of the ratio and linear-combination operations has floating point (CV_32F) precision unless the input is already in doubles (if so, precision is unchanged)\n";
    oss << "\n";
    oss << "    cv::Mat image operations\n";
    oss << "        simple-blob[=<parameters>]: wraps cv::SimpleBlobDetector, outputs as csv key points timestamped by image timestamp\n";
    oss << "            <parameters>\n";
    oss << "                output-binary: output key points as binary\n";
    oss << "                output-fields: print output fields on stdout and exit\n";
    oss << "                output-format: print binary output format on stdout and exit\n";
    oss << "                output-default-params,default-params: print default simple blob detector parameters to stdout and exit\n";
    oss << "                <filename>[:<path>]: simple blob detector params config file and optionally cv file node path in it\n";
    oss << "                                     e.g: my-params.txt:blob\n";
    oss << "                                     if not present, defaults will be used\n";
    oss << "\n";
    oss << snark::imaging::vegetation::impl::filters< H >::usage() << "\n";
    return oss.str();
}
