...
static void usage( bool verbose )
{
    ...
    std::cerr << "    math-deg2rad <angle> [--precision=<n>]" << std::endl;
...
int main( int ac, char** av )
{
    ...
    catch( std::exception& ex ) { std::cerr << "math-deg2rad: " << ex.what() << std::endl; }
    catch( ... ) { std::cerr << "math-deg2rad: unknown exception" << std::endl; }
    return 1;
}
